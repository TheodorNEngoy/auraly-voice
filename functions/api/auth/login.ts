export const onRequestGet: PagesFunction<{ DB: D1Database, GOOGLE_CLIENT_ID: string, PUBLIC_OAUTH_BASE: string }> = async ({ env, request }) => {
  const clientId = (env as any).GOOGLE_CLIENT_ID as string;

  // Always build the redirect URI from a configured base (production host)
  const base = (env as any).PUBLIC_OAUTH_BASE || new URL(request.url).origin;
  const redirectUri = new URL("/api/auth/callback", base).toString();
